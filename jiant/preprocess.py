...
def add_transformers_vocab(vocab, tokenizer_name):
    ...
    elif tokenizer_name.startswith("visualbert"):
        tokenizer = BertTokenizer.from_pretrained("bert-base-uncased", do_lower_case=do_lower_case)
    elif tokenizer_name.startswith("lxmert"):
        tokenizer = LxmertTokenizer.from_pretrained("unc-nlp/lxmert-base-uncased", do_lower_case=True)
    elif tokenizer_name.startswith("clip"):
    ...
    if (
        ...
        tokenizer.add_special_tokens(
            {"mask_token": "<|mask|>", "sep_token": "<|sep|>", "cls_token": "<|cls|>", "unk_token": "<|unk|>"}
        )
...
class ModelPreprocessingInterface(object):
    ...
    def __init__(self, args):
        ...
        elif args.input_module.startswith("visualbert"):
            from jiant.huggingface_transformers_interface.modules import VisualBertEmbedderModule

            boundary_token_fn = VisualBertEmbedderModule.apply_boundary_tokens
        
        elif args.input_module.startswith("lxmert"):
            from jiant.huggingface_transformers_interface.modules import LXMERTEmbedderModule

            boundary_token_fn = LXMERTEmbedderModule.apply_boundary_tokens

        elif args.input_module.startswith("clip"):
            from jiant.huggingface_transformers_interface.modules import CLIPEmbedderModule

            boundary_token_fn = CLIPEmbedderModule.apply_boundary_tokens
            lm_boundary_token_fn = CLIPEmbedderModule.apply_lm_boundary_tokens
