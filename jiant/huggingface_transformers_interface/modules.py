...
class HuggingfaceTransformersEmbedderModule(nn.Module):
    ...
    def parameter_setup(self, args):
        ...
        if self.model.config._name_or_path == 'unc-nlp/lxmert-base-uncased':
            self.num_layers = self.model.config.num_hidden_layers["language"] + self.model.config.num_hidden_layers["cross_encoder"]
    ...
    def prepare_output(self, lex_seq, hidden_states, input_mask):
        ...
        #print("available_layers",available_layers)
...
class BertEmbedderModule(HuggingfaceTransformersEmbedderModule):
    ...
    def forward(self, sent: Dict[str, torch.LongTensor], task_name: str = "") -> torch.FloatTensor:
        ...
        if self.output_mode != "only":
            ...
            output = self.model(ids, token_type_ids=token_types, attention_mask=input_mask)
...
class VisualBertEmbedderModule(HuggingfaceTransformersEmbedderModule):
    """ Wrapper for VisualBERT module to fit into jiant APIs.
    Check HuggingfaceTransformersEmbedderModule for function definitions """
    def __init__(self, args):
        super(VisualBertEmbedderModule, self).__init__(args)

        self.model = transformers.VisualBertModel.from_pretrained(
            "uclanlp/visualbert-vqa-coco-pre", cache_dir=self.cache_dir, output_hidden_states=True
        )
        self.max_pos = self.model.config.max_position_embeddings

        self.tokenizer = transformers.BertTokenizer.from_pretrained(
            "bert-base-uncased", cache_dir=self.cache_dir, do_lower_case=True
        )  # TODO: Speed things up slightly by reusing the previously-loaded tokenizer.
        self._sep_id = self.tokenizer.convert_tokens_to_ids("[SEP]")
        self._cls_id = self.tokenizer.convert_tokens_to_ids("[CLS]")
        self._pad_id = self.tokenizer.convert_tokens_to_ids("[PAD]")
    ...
    def forward(self, sent: Dict[str, torch.LongTensor], task_name: str = "") -> torch.FloatTensor:
        ...
        if self.output_mode != "only":
            ...
            # zero_vec=torch.zeros(1,37,2048)
            image_flag=True
            token_types = self.get_seg_ids(ids, input_mask)
            black_image = torch.load("")
            black_image = torch.cat([black_image]*len(ids)).to('cuda')
            visual_attention_mask=torch.ones(len(ids),37).to('cuda')
            visual_token_type_ids=torch.zeros([len(ids),37],dtype=torch.long).to('cuda')
            #batch size分の画像を用意

            outputs = self.model(input_ids=ids, attention_mask=input_mask, token_type_ids=token_types,
                    visual_embeds=black_image, visual_attention_mask=visual_attention_mask, visual_token_type_ids=visual_token_type_ids)
            
            
            
            if image_flag:
                hidden_states = (outputs["hidden_states"][0][:,:-37,:],outputs["hidden_states"][1][:,:-37,:],outputs["hidden_states"][2][:,:-37,:],
                    outputs["hidden_states"][3][:,:-37,:],outputs["hidden_states"][4][:,:-37,:],
                    outputs["hidden_states"][5][:,:-37,:],outputs["hidden_states"][6][:,:-37,:],outputs["hidden_states"][7][:,:-37,:],
                    outputs["hidden_states"][8][:,:-37,:],outputs["hidden_states"][9][:,:-37,:],outputs["hidden_states"][10][:,:-37,:],
                    outputs["hidden_states"][11][:,:-37,:],outputs["hidden_states"][12][:,:-37,:])
            else:
    ...
    """
    def get_pretrained_lm_head(self):
        model_with_lm_head = transformers.VisualBertModel.from_pretrained(
            "uclanlp/visualbert-vqa-coco-pre", cache_dir=self.cache_dir
        )
        lm_head = model_with_lm_head.cls
        lm_head.predictions.decoder.weight = self.model.embeddings.word_embeddings.weight
        return lm_head
...
class LXMERTEmbedderModule(HuggingfaceTransformersEmbedderModule):
    """ Wrapper for LXMERT module to fit into jiant APIs.
    Check HuggingfaceTransformersEmbedderModule for function definitions """
    def __init__(self, args):
        super(LXMERTEmbedderModule, self).__init__(args)

        self.model = transformers.LxmertModel.from_pretrained(
            "unc-nlp/lxmert-base-uncased", cache_dir=self.cache_dir, output_hidden_states=True
        )
        self.max_pos = self.model.config.max_position_embeddings

        self.tokenizer = transformers.LxmertTokenizer.from_pretrained(
            "unc-nlp/lxmert-base-uncased", cache_dir=self.cache_dir, do_lower_case=True
        )  # TODO: Speed things up slightly by reusing the previously-loaded tokenizer.
        self._sep_id = self.tokenizer.convert_tokens_to_ids("[SEP]")
        self._cls_id = self.tokenizer.convert_tokens_to_ids("[CLS]")
        self._pad_id = self.tokenizer.convert_tokens_to_ids("[PAD]")
    ...
    def forward(self, sent: Dict[str, torch.LongTensor], task_name: str = "") -> torch.FloatTensor:
        ...
        if self.output_mode != "only":
            ...
            black_image = torch.load("your/image.pt").to('cuda')
            visual_pos = torch.ones(1,37,4).to("cuda")
            output = self.model(ids, attention_mask=input_mask,token_type_ids=token_types,
                                visual_feats=black_image,visual_pos = visual_pos
                                )
            #this hidden states include 9 language layers and 5 cross modal layers
            hidden_states = output["language_hidden_states"]
...
class CLIPEmbedderModule(HuggingfaceTransformersEmbedderModule):
    """ Wrapper for CLIPText module to fit into jiant APIs.
    Check HuggingfaceTransformersEmbedderModule for function definitions """
    def __init__(self, args):
        super(CLIPEmbedderModule, self).__init__(args)

        self.model = transformers.CLIPTextModel.from_pretrained(
            "openai/clip-vit-base-patch32", cache_dir=self.cache_dir, output_hidden_states=True
        )
        self.max_pos = self.model.config.max_position_embeddings
        ...
        special_tokens = {"mask_token": "<|mask|>", "sep_token": "<|sep|>", "cls_token": "<|cls|>", "unk_token": "<|unk|>"}
    ...
    @staticmethod
    def apply_lm_boundary_tokens(s1, get_offset=False):
        # CLIP style boundary token marking on string token sequences for LM tasks
    ...
    def forward(self, sent: Dict[str, torch.LongTensor], task_name: str = "") -> torch.FloatTensor:
        ...
        if self.output_mode != "only":
            #token_types = self.get_seg_ids(ids, input_mask)
            output = self.model(ids, attention_mask=input_mask)
            hidden_states = output["hidden_states"]
